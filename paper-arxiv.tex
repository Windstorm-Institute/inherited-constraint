% Paper 6 of 7 in The Windstorm Series
...
\texttt{grant@windstorminstitute.org}}
...
Natural language (BPT $\sim 3.90$) falls inside the throughput basin. The ribosome processes information at 4.39 bits per codon. Claude Shannon, in 1951, estimated the entropy of printed English at approximately 1 bit per character---roughly 5 bits per word~\cite{shannon1951}. Three independent measurements, three different methods, three different decades, the same neighborhood.
...
\textbf{Link~2 (Established): Biology constrains cognition.} Neural systems built from biological substrates inherit the throughput constraint. Working memory: approximately 3.1 bits per item. Phoneme discrimination: approximately 4.2 bits. Evidence: Miller~\cite{miller1956}, Miller and Nicely~\cite{miller1955}, decades of psychophysics.

\textbf{Link~3 (Proposed): Cognition constrains language.} Language evolved as a communication channel between brains constrained to approximately 4--5 bits per event. Evidence: BPT $\sim 4.4$ (this paper), Shannon's 1951 estimate~\cite{shannon1951}, the UID literature~\cite{jaeger2010}. Not yet causally demonstrated.

\textbf{Link~4 (Proposed): Language constrains AI.} Models trained on human language learn its statistical structure. The approximately 4.4-bit BPT reflects the information density that biological cognition selected for. Evidence: the shuffling experiment. Not yet tested with a synthetic-training control.
...
\begin{enumerate}
\item \textbf{Cross-linguistic universality:} Shuffled non-English text (Chinese, Arabic, Finnish) should produce BPT of approximately 5 bits ($\pm 1$ for vocabulary effects). Falsification: shuffled Pirah\~a produces BPT outside 3--7 bits.
...
\bigskip
\noindent\emph{This paper is Paper~6 of The Windstorm Series. The series includes Paper~1: The Fons Constraint~\cite{whitmer2026fons}, Paper~2: The Receiver-Limited Floor~\cite{whitmer2026floor}, Paper~3: The Throughput Basin~\cite{whitmer2026basin}, Paper~4: The Serial Decoding Basin~\cite{whitmer2026serial}, Paper~5: The Dissipative Decoder~\cite{whitmer2026dissipative}, and Paper~7: The Throughput Basin Origin (DOI: 10.5281/zenodo.19498582).}
